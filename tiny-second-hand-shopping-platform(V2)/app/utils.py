...
def redirect_back_or_default(default_endpoint, **values):
...
def create_default_admin():
    ...
    admin.display_name = current_app.config["ADMIN_DISPLAY_NAME"]
    admin.bio = "시스템 초기 관리자 계정입니다."
    admin.is_admin = True
...
def log_admin_action(admin_user, action_type, target_type, target_id, details=""):
    ...
    log.details = sanitize_text(details, 300)
    db.session.add(log)
...
def get_favorite_product_ids(user_id):
    if not user_id:
        return set()
    rows = db.session.query(ProductFavorite.product_id).filter_by(user_id=user_id).all()
    return {product_id for (product_id,) in rows}


def get_product_favorite_counts(product_ids):
    normalized_ids = [
        product_id for product_id in dict.fromkeys(product_ids or []) if product_id
    ]
    if not normalized_ids:
        return {}
    rows = (
        db.session.query(ProductFavorite.product_id, func.count(ProductFavorite.id))
        .filter(ProductFavorite.product_id.in_(normalized_ids))
...
def mark_direct_messages_read(thread_key, reader_id):
    unread_messages = (
    ...
    db.session.commit()
    return unread_messages, read_at
